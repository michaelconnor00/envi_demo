PRO ISODATA, input_location, output_location
    ...
    image_dir = Filepath('', ROOT_DIR=input_dir)
    files = File_Search(image_dir, '*.TIF', /FULLY_QUALIFY_PATH)
    ...
    FOR I=1, file_list_size DO BEGIN
        ...
        out_put_path = output_dir + STRTRIM(STRING(I), 1) + '_output.tif'

        e.ExportRaster, Task.OUTPUT_RASTER, out_put_path, "TIFF"
    ENDFOR

    output_image_dir = Filepath('', ROOT_DIR=output_dir)
    ...
    out_put_path = output_dir + 'cd_output.tif'
